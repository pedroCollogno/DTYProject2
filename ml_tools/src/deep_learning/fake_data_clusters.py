...
def fake_data_generator(sequence_size, n_clusters, avg_n_emittors_in_clusters, silence_time):
    ...
    for i in range(n_clusters):
        ...
        rnd_array = np.random.multinomial(sum, np.concatenate((np.ones(n-1)/(n+silence_time-1), np.array(
            [silence_time/(n+silence_time-1)]))), size=1)[0]  # 20 corresponds to the silence time in each sequence
        ...
        emissions = {}
        for k in range(1, n):
            emissions["cluster_"+str(i)+"_emittor_" +
...
def create_cluster_comparison(sequence_size, n_clusters, avg_n_emittors_in_clusters, silence_time):
    ...
    clusters = fake_data_generator(
        sequence_size, n_clusters, avg_n_emittors_in_clusters, silence_time)
    labels = []
    data_for_deep = []
            ...
            for cluster_secondary in clusters:
                ...
                for emittor_secondary in cluster_secondary:
                    if (emittor_secondary != emittor):
                        data = [int(data[k] or cluster_secondary[emittor_secondary][k])
