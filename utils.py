...
def display_alternates(*args):
    ...
    if len(args) > 1:
        ...
        y_labels = []
        y_ticks = []
        for key in all_alternates["prp{}".format(1)]:
            boxes = []
            for alternate in all_alternates["prp{}".format(1)][key]:
                boxes.append((alternate.start.date_ms/(1000),
                            alternate.duration_us/1000000))
            ax.broken_barh(boxes, (key_indice, 0.9), facecolors='blue')
            y_ticks.append(key_indice)
            y_labels.append(str(key))
            key_indice += 1
...
if __name__ == '__main__':
    ...
    prp_test = "prod/test_display.prp"
    display_alternates(prp_test)
